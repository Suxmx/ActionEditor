...
using UnityEditor;
using TickHandler = M2.Editor.Bridge.TickHandler;
...
namespace SimpleTimeArea.Editor
{
    ...
    public class TimeArea : ZoomableArea
    {
        ...
        public const int kTickRulerDistMin = 3; // min distance between ruler tick marks before they disappear completely

        public const int kTickRulerDistFull = 80; // distance between ruler tick marks where they gain full strength
        public const int kTickRulerDistLabel = 40; // min distance between ruler tick mark labels
        public const float kTickRulerHeightMax = 0.7f; // height of the ruler tick marks when they are highest

        public const float kTickRulerFatThreshold = 0.5f; // size of ruler tick marks at which they begin getting fatter
        ...
        public List<float> _majorTickXs = new();
            ...
            for (int l = 0; l < hTicks.tickLevels; l++)
            {
                ...
                if (strength > kTickRulerFatThreshold)
                {
                    ...
                    for (int i = 0; i < s_TickCache.Count; i++)
                    {
                        ...
                        // Draw line
                        _majorTickXs.Add(x);
                        GUIBridge.DrawVerticalLine(x, 0.0f, position.height, tickColor);
                    }
                }
            }
            ...
            if (Event.current.type == EventType.Repaint)
            {
                ...
                for (int l = 0; l < hTicks.tickLevels; l++)
                {
                    ...
                    for (int i = 0; i < s_TickCache.Count; i++)
                    {
                        ...
                        // Draw line
                        GUIBridge.DrawVerticalLine(x, position.height - height + 0.5f, position.height - 0.5f,
        ...
        public static void DrawVerticalLine(float x, float minY, float maxY, Color color)
        {
            ...
            if (Application.platform == RuntimePlatform.WindowsEditor)
                ...
                GL.Begin(GL.LINES);
            GUIBridge.DrawVerticalLine(x, minY, maxY, color);
            GL.End();
