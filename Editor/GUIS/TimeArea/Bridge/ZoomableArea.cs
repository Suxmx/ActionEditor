...
namespace SimpleTimeArea.Editor
{
    ...
    public class ZoomableArea
    {
        ...
        void HandleScrolling(Rect area)
        {
            if (m_MinimalGUI)
                return;
            if (m_VSlider && new Rect(area.x + area.width, area.y + GUI.skin.verticalScrollbarUpButton.fixedHeight, vSliderWidth,
                    area.height - (GUI.skin.verticalScrollbarDownButton.fixedHeight + hSliderHeight)).Contains(Event.current.mousePosition))
            {
            ...
            if (m_HSlider && new Rect(area.x + GUI.skin.horizontalScrollbarLeftButton.fixedWidth, area.y + area.height,
                    area.width - (GUI.skin.horizontalScrollbarRightButton.fixedWidth + vSliderWidth), hSliderHeight).Contains(Event.current.mousePosition))
            {
        ...
        public void HSliderGUI(Rect r)
        {
            ...
            {
                EditorGUIExt.MinMaxScroller(r, horizontalScrollbarID,
            ...
            {
                shownXMin = GUI.HorizontalScrollbar(
                    r, // 滚动条的位置和尺寸
                    shownXMin, // 当前滚动条的起始值
                    shownXRange, // 滚动条的长度
                    editorBounds.min.x, // 滚动条的最小值
                    editorBounds.max.x // 滚动条的最大值
                );
            }
            ...
            using (new EditorGUI.DisabledScope(!enableMouseInput))
            {
                ...
                if (m_VSlider)
                {
                    if (m_UpDirection == YDirection.Positive)
                    {
                        ...
                        {
                            shownYMin = GUIBridge.Scroller(vRangeSliderRect,
